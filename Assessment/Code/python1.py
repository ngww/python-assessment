...
def two(input):
	...
	pos_a = lower.find("bert")
	if pos_a == -1:
		return ""

	pos_b = lower.rfind("bert")
	if pos_b == -1:
		return ""

	new_pos_a = pos_a + len("bert")
	if new_pos_a >= pos_b:
		return ""
	
	value = input[new_pos_a:pos_b]
	return value
...
def nine(inputString, char):
	inputString2 = inputString.replace(" ", "")
	pos = None
	for i in range(0, len(inputString2)):
		if  inputString2[i] == char:
			pos = i + 1
			break
	if pos == None:
		return -1
